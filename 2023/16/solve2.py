...
def simulate_beam(grid, start_x, start_y, direction):
    energized_tiles = set()
    to_visit = [(start_x, start_y, direction)]
    visited = set()
...
def find_starting_pos(grid):
    ...
    for y in range(len(grid[0])):
        max_energized_tiles = max(max_energized_tiles, simulate_beam(grid, 0, y, DOWN))

    # Bottom row, heading upward
    for y in range(len(grid[0])):
        max_energized_tiles = max(
            max_energized_tiles, simulate_beam(grid, len(grid) - 1, y, UP)
        )

    # Leftmost column, heading right
    for x in range(len(grid)):
        max_energized_tiles = max(max_energized_tiles, simulate_beam(grid, x, 0, RIGHT))

    # Rightmost column, heading left
    for x in range(len(grid)):
        max_energized_tiles = max(
            max_energized_tiles, simulate_beam(grid, x, len(grid[0]) - 1, LEFT)
        )

    return max_energized_tiles


total = find_starting_pos(inp)
print(total)
